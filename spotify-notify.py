...
import gobject, gtk, os, tempfile, sys, time, re, indicate, urllib2, signal
...
class SpotifyNotify():
    ...
    def __init__(self, debugger):
        ...
        self.mixers = [alsaaudio.Mixer(name, cardindex=0) for name in ["Master", "Speaker", "Headphone"]]
    ...
    def trackChange(self, *trackChange):
        ...
        if 'http:' in trackInfo['album'][:5] or 'spotify' in trackInfo['album'][:7]:
            Debug.out("Should mute volume now")
            for mixer in self.mixers:
                mixer.setmute(1, alsaaudio.MIXER_CHANNEL_ALL)
        else:
            Debug.out("Should unmute volume now")
            for mixer in self.mixers:
                mixer.setmute(0, alsaaudio.MIXER_CHANNEL_ALL)
    ...
    def startSpotify(Debug):
        ...
        procs = SpotifyNotify.checkForProcess(
            'ps x | grep "spotify"'.format(ident),
            Debug
        )
        if len(procs):
...
# Catches SIGUSR1 signals and reposts the notification
def post_notify(signum, frame):
    global SN
    if SN is not None:
        SN.renotify()
...
if __name__ == "__main__":
    ...
    signal.signal(signal.SIGUSR1, post_notify)
